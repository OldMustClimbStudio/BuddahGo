...
public class HandPushProjectileRuntime : MonoBehaviour
{
        ...
        bool ignoreSolidWorld = false)
    {
        ...
        projectile.InitializeServer(attacker, startPos, direction, speed, lifetimeSeconds, impulse, colliderSize, hitTurnTorqueImpulse, usingGeneratedCollider, allowAttackerHit, ignoreSolidWorld);
        return projectile;
    }
    ...
    private void Update()
    {
        ...
        if (!_ignoreSolidWorld && TryStopAtSolidWorld(moveDistance))
            return;
    ...
    private bool TryStopAtSolidWorld(float moveDistance)
    {
        ...
        Vector3 blockedPosition = transform.position + (_direction * Mathf.Max(0f, nearestDistance - 0.02f));
        blockedPosition.y = _lockedY;
    ...
    private bool ShouldIgnoreSolidHit(Collider hitCollider)
    {
        ...
        if (_attacker != null && hitCollider.transform.IsChildOf(_attacker.transform))
            return true;

        if (hitCollider.GetComponentInParent<NetworkObject>() != null)
            return true;
