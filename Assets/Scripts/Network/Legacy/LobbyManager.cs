...
public class LobbyManager : NetworkBehaviour
{
    ...
    public bool AllPlayersReady => AreAllPlayersReady();
    ...
    public bool TryGetPlayer(int clientId, out LobbyPlayer player)
    {
        int index = FindPlayerIndex(clientId);
        if (index >= 0)
        {
    ...
    private void UpdatePlayersListText()
    {
        ...
        for (int i = 0; i < Players.Count; i++)
        {
            ...
            string readyLabel = player.Ready ? "Ready" : "Waiting";
            sb.AppendLine($"{player.DisplayName} [{readyLabel}]");
        }
